...
function CasesLoadingState() {
  return <section aria-busy="true" aria-labelledby="portal-counseling-cases-loading-heading" className="portal-counseling portal-counseling--loading" role="status"><span className="sr-only">Loading counseling cases…</span><PortalPageHeader className="portal-counseling__page-header" current="Counseling" description="Review case metadata within your authorized scope." headingId="portal-counseling-cases-loading-heading" title="Counseling" /><div className="portal-counseling__workspace"><div aria-hidden="true" className="compass-surface portal-workspace-nav portal-counseling__nav-skeleton" data-tone="subtle">{COUNSELING_NAV_ITEMS.map((item) => <Skeleton className="portal-counseling__nav-skeleton-line" key={item.value} />)}</div><div className="portal-counseling__content-skeleton"><PortalFilterPanel action="/portal/counseling?section=cases" ariaBusy className="portal-counseling__filters" resetKey="case-loading" summary="Loading filters…"><div aria-hidden="true" className="portal-counseling__filter-skeleton-grid">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} />)}</div></PortalFilterPanel><PortalCollectionFrame className="portal-counseling__frame"><div aria-hidden="true" className="portal-counseling__table-skeleton">{Array.from({ length: 5 }, (_, row) => <div className="portal-counseling__table-skeleton-row" key={row}>{Array.from({ length: 8 }, (_, cell) => <Skeleton as="span" key={cell} />)}</div>)}</div></PortalCollectionFrame></div></div></section>;
}

function CasesState({ kind, error, onRetry }: { kind: "forbidden" | "unavailable"; error?: LoadState["kind"] | "rate_limited" | "validation"; onRetry?: () => void }) {
  ...
  return <section aria-labelledby="portal-counseling-cases-state-heading" className="portal-counseling portal-counseling--state" role={kind === "unavailable" ? "alert" : undefined}><PortalPageHeader className="portal-counseling__page-header" current="Counseling" description="Review case metadata within your authorized scope." headingId="portal-counseling-cases-state-heading" title="Counseling" /><div className="portal-counseling__workspace"><PortalWorkspaceNav activeValue="cases" ariaLabel="Counseling sections" items={COUNSELING_NAV_ITEMS} /><PortalCollectionFrame className="portal-counseling__frame portal-counseling__frame--state"><HeartHandshake aria-hidden="true" className="portal-counseling__state-icon" /><h2>{title}</h2><p>{kind === "forbidden" ? "Return to your workspace to continue." : "Try again when the connection is ready."}</p>{onRetry ? <Button onClick={onRetry} type="button" variant="outline"><RefreshCw aria-hidden="true" />Try again</Button> : null}</PortalCollectionFrame></div></section>;
}

export function CounselingCasesView({ hasCapability }: { hasCapability: (capability: string) => boolean }) {
