...
type DocumentDialog = "template" | "version" | null;

export function GuidanceInstitutionDocuments({ canTemplates }: InstitutionDocumentsProps) {
  ...
  const submitDialog = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!dialog) return;
    const fingerprint = JSON.stringify({ dialog, templateTarget, versionTarget, templateFields, versionFields });
  ...
  const dialogTitle = dialog === "template" ? (templateTarget ? "Edit document template" : "New document template") : "Edit template version";
  ...
  return (
    <div className="portal-counseling__stack">
          ...
          {templates.kind === "loading" ? <><Skeleton as="span" /><Skeleton as="span" /></> : templates.kind === "error" ? <StateBlock retry={reload} title="Document templates are temporarily unavailable." /> : templates.value.length === 0 ? <p>No document templates are available.</p> : (
            <div className="portal-counseling__table-wrap"><table className="portal-counseling__table"><thead><tr><th scope="col">Template</th><th scope="col">Kind / format</th><th scope="col">Retention</th><th scope="col">Status</th><th scope="col">Actions</th></tr></thead><tbody>
              {templates.value.map((item) => {
                const versionState = versions[item.stable_key];
                const hasPublishedVersion = versionState?.kind === "ready" && versionState.value.some((version) => version.status === "PUBLISHED");
                const canArchiveTemplate = item.status === "DRAFT" || (item.status === "PUBLISHED" && versionState?.kind === "ready" && !hasPublishedVersion);
                return (
                  <tr key={item.stable_key}>
                    ...
                    <Cell label="Kind / format">{label(item.document_kind)} · {label(item.default_output_format)}</Cell>
                    <Cell label="Retention">{label(item.retention_classification)}</Cell>
                    <Cell label="Status"><Badge>{label(item.status)}</Badge></Cell>
                    <Cell label="Actions">
                      <div className="portal-counseling__row-actions">
                        <ActionButton disabled={busy !== null} label={expandedTemplate === item.stable_key ? "Hide versions" : "Show versions"} onClick={() => loadVersions(item)} />
                        {item.status === "DRAFT" ? <ActionButton disabled={busy !== null} label="Edit" onClick={() => openTemplate(item)} /> : null}
                        {canArchiveTemplate ? <ActionButton disabled={busy !== null} label="Archive" onClick={() => void mutate(`template-archive-${item.stable_key}`, (key) => documentTemplateLifecycle(item, "archive", { expected_status: item.status }, key))} /> : null}
                      </div>
                      {expandedTemplate === item.stable_key ? (
                        <div className="portal-counseling__expanded-panel">
                          {!versionState || versionState.kind === "loading" ? <p>Loading template versions…</p> : versionState.kind === "error" ? <StateBlock title="Template versions are temporarily unavailable." /> : versionState.value.length === 0 ? <p>No versions are available.</p> : (
                            <ul>
                              {versionState.value.map((version) => (
                                <li key={`${version.version_label}-${version.status}`}>
                                  <span><strong>{version.version_label}</strong> · {label(version.status)}{version.is_used ? " · In use" : ""}</span>
                                  <span className="portal-counseling__row-actions">
                                    {version.status === "DRAFT" ? <ActionButton disabled={busy !== null} label="Edit" onClick={() => openVersion(version)} /> : null}
                                    <ActionButton disabled={busy !== null} label="Preflight" onClick={() => void inspectPreflight(version)} />
                                    {version.status === "DRAFT" ? <ActionButton disabled={busy !== null || preflight?.version !== version || !preflight.ready} label="Publish" onClick={() => void mutate(`version-publish-${version.version_label}`, (key) => templateVersionLifecycle(version, "publish", { expected_status: version.status }, key))} /> : null}
                                    {version.status === "PUBLISHED" || version.status === "ARCHIVED" ? <ActionButton disabled={busy !== null} label="Clone" onClick={() => void mutate(`version-clone-${version.version_label}`, (key) => templateVersionLifecycle(version, "clone", { expected_status: version.status }, key))} /> : null}
                                    {version.status !== "ARCHIVED" ? <ActionButton disabled={busy !== null} label="Archive" onClick={() => void mutate(`version-archive-${version.version_label}`, (key) => templateVersionLifecycle(version, "archive", { expected_status: version.status }, key))} /> : null}
                                  </span>
                                </li>
                              ))}
                            </ul>
      ...
      {preflight ? <PortalCollectionFrame className="portal-counseling__frame"><div className="portal-counseling__frame-heading"><div><p className="portal-counseling__kicker">Template publication preflight</p><h2>{preflight.version.version_label}</h2></div><Badge>{preflight.ready ? "Ready" : "Blocked"}</Badge></div>{preflight.blockers.length ? <ul>{preflight.blockers.map((blocker) => <li key={blocker}>{label(blocker)}</li>)}</ul> : <p>This version passed the current publication checks.</p>}</PortalCollectionFrame> : null}

      <AlertDialog onOpenChange={(open) => { if (!open && busy === null) { mutationKeys.current.delete("template-dialog"); mutationKeys.current.delete("version-dialog"); setDialog(null); setTemplateTarget(null); setVersionTarget(null); setError(null); } }} open={dialog !== null}>
...
export function GuidanceCounselorCoverage({ onChanged }: CoverageProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editTarget) await updateGuidanceCoverage(editTarget, { ...payload, expected_resource_version: editTarget.resource_version }, key);
