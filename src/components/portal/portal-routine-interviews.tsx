...
export function RoutineInterviewsLoadingState() {
  return <section aria-busy="true" aria-labelledby="portal-counseling-routine-loading-heading" className="portal-counseling portal-counseling--loading" role="status"><span className="sr-only">Loading routine interviews…</span><PortalPageHeader className="portal-counseling__page-header" current="Counseling" description="Review routine interviews within your authorized scope." headingId="portal-counseling-routine-loading-heading" title="Counseling" /><div className="portal-counseling__workspace"><div aria-hidden="true" className="compass-surface portal-workspace-nav portal-counseling__nav-skeleton" data-tone="subtle">{COUNSELING_NAV_ITEMS.map((item) => <Skeleton className="portal-counseling__nav-skeleton-line" key={item.value} />)}</div><div className="portal-counseling__content-skeleton"><PortalFilterPanel action="/portal/counseling?section=routine-interviews" ariaBusy className="portal-counseling__filters" resetKey="routine-loading" summary="Loading filters…"><div aria-hidden="true" className="portal-counseling__filter-skeleton-grid"><Skeleton /><Skeleton /></div></PortalFilterPanel><PortalCollectionFrame className="portal-counseling__frame"><div aria-hidden="true" className="portal-counseling__table-skeleton">{Array.from({ length: 5 }, (_, row) => <div className="portal-counseling__table-skeleton-row portal-counseling__routine-table-skeleton-row" key={row}>{Array.from({ length: 6 }, (_, cell) => <Skeleton as="span" key={cell} />)}</div>)}</div></PortalCollectionFrame></div></div></section>;
}
